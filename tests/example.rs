...
async fn bridge_end_to_end_test(http_client: &Arc<HttpClient>, replicator: &Arc<Replicator>) {
    ...
    assert!(res.err().unwrap().to_string().contains("Withdrawal not signed by GX Exchange."));
...
async fn spawn_hardhat_node() {
    std::thread::spawn(|| {
        shell("(cd ~/cham/code/gx-exchange && npx hardhat node) > /tmp/hardhat_out 2>&1".to_string()).wait_check()
    });
    lu::async_sleep(Duration(1.)).await;
    shell("(cd ~/cham/code/gx-exchange && npx hardhat run deploy/contracts.js --network localhost) > /tmp/hardhat_deploy_out 2>&1".to_string()).wait_check();
}
